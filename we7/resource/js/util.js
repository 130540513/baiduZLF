...
util.request = function (option) {
  ...
  var sessionid = swan.getStorageSync('userInfo').sessionid;
  var url = option.url;
  ...
  if (option.cachetime) {
    ...
    var cachedata = swan.getStorageSync(cachekey);
    ...
    if (cachedata && cachedata.data) {
      if (cachedata.expire > timestamp) {
        ...
        swan.removeStorageSync(cachekey);
      }
    }
  }
  swan.request({
    ...
    'success': function (response) {
      ...
      if (response.data.errno) {
        ...
        if (option.cachetime) {
          ...
          swan.setStorageSync(cachekey, cachedata);
        }
      }
    },
...
util.getUserInfo = function (cb) {
  ...
  var app = swan.getStorageSync('userInfo');
  if (app.sessionid) {
    swan.checkSession({
      success: function () {
...
util.showLoading = function () {
  var isShowLoading = swan.getStorageSync('isShowLoading');
  if (isShowLoading) {
    swan.hideLoading();
    swan.setStorageSync('isShowLoading', false);
  }

  swan.showLoading({
    title: '加载中',
    complete: function () {
      swan.setStorageSync('isShowLoading', true);
    },
    fail: function () {
      swan.setStorageSync('isShowLoading', false);
    }
